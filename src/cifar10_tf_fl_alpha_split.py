...
def build_deterministic_train_ds(train_images, train_labels, batch_size):
    ds = tf.data.Dataset.from_tensor_slices((train_images, train_labels))
    # Use evaluation-time preprocessing to avoid random augmentations
    ds = preprocess_dataset(ds, is_training=False, batch_size=batch_size)
    return ds

def dataset_to_matrix(ds):
    xs = []
    for x, _ in ds:
        # x is already normalized by CIFAR-10 mean/std in preprocess_dataset
        x = tf.reshape(x, [x.shape[0], -1])  # flatten to [B, 3072]
        xs.append(x.numpy())
...
def compute_centroid_from_train(model, train_images, train_labels, batch_size, feature_layer_name):
    ds = tf.data.Dataset.from_tensor_slices((train_images, train_labels))
    ds = preprocess_dataset(ds, is_training=False, batch_size=batch_size) # deterministic, no aug
    # Build feature extractor; add GAP if needed
    feat_tensor = model.get_layer(feature_layer_name).output
    if len(feat_tensor.shape) == 4:
        gap = tf.keras.layers.GlobalAveragePooling2D(name=feature_layer_name + "_gap")(feat_tensor)
        extractor = tf.keras.Model(inputs=model.inputs, outputs=gap)
    else:
      extractor = tf.keras.Model(inputs=model.inputs, outputs=feat_tensor)
    feats = []
    for x, _ in ds:
        f = extractor(x, training=False)
        feats.append(f.numpy())
    F = np.concatenate(feats, axis=0) # [N, D_feat]
    centroid = F.mean(axis=0).astype(np.float32) # no per-client centering
    return centroid
def main():
    ...
    while flare.is_running():
        ...
        mlflow_writer.log_metric(key="global_model_accuracy", value=test_global_acc, step=input_model.current_round)
        feature_layer_name = 'dense_1'  # e.g., "dense_1" or similar
        centroid_pre = compute_centroid_from_train(
            model=model,
        ...
        mlflow_writer.log_metric(key="local_model_accuracy", value=test_acc, step=input_model.current_round)
        ...
        output_model = flare.FLModel(
            ...
            , meta={"centroid": centroid_pre.tolist()}
        )
